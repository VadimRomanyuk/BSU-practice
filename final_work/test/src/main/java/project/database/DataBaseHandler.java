...
public class DataBaseHandler implements IPostService {
    ...
    public void signUser(User user)
    {
       ...
       {
           prSt.setInt(1,getId(user.getID()));
        ...
        try( PreparedStatement prSt = dbConnect.prepareStatement(sql))
        {
            ...
            while(res.next())
            {
                users.add(new User(res.getString(DBConsts.USER_NAME),String.valueOf(res.getInt(DBConsts.USER_ID)),
                        res.getString(DBConsts.USER_PASS)));
            }
    ...
    public void savePost(String userId, PhotoPost post) {
        ...
        try(PreparedStatement prSt = dbConnect.prepareStatement(sql))
        {
           ...
           prSt.setString(6,new Gson().toJson(post.getLikes()));
           prSt.setString(7,new Gson().toJson(post.getHashtags()));
           prSt.executeUpdate();
        }
        catch (Exception e)
        {
    ...
    public PhotoPost getPost(String author , String id) {
        ...
        try(PreparedStatement prSt = dbConnect.prepareStatement(sql))
        {
            ...
            String[] tags = new Gson().fromJson(res.getString(DBConsts.PHOTO_POST_TAGS),String[].class);
            String[] likes = new Gson().fromJson(res.getString(DBConsts.PHOTO_POST_LIKES),String[].class);
            ArrayList<String>  likeres = new ArrayList<>(Arrays.asList(likes));
            ArrayList<String>  tagsres = new ArrayList<>(Arrays.asList(tags));
            PhotoPost post = new PhotoPost(post_id,descr,author,link,createdAt,likeres,tagsres);
            return post;
        }
        catch (Exception e)
        {
    ...
    public List<PhotoPost> getAllPosts() {
        ...
        try(PreparedStatement prSt = dbConnect.prepareStatement(sql))
        {
            ...
            while(res.next())
            {
                ...
                for(User user : users)
                {
                    if(user.getID().equals(String.valueOf(res.getInt(DBConsts.USER_ID))))
                    {
                        author = user.getNAME();
                        break;
                    }
    ...
    public void editPost(PhotoPost post)
    {
        ...
        sql.append(DBConsts.PHOTO_POST_TAGS + " = ? ");
        ...
        try(PreparedStatement prSt = dbConnect.prepareStatement(sql.toString()))
        {
            ...
            prSt.setString(3,new Gson().toJson(post.getHashtags()));
            prSt.executeUpdate();
        }
        catch (Exception e)
        {
    ...
    public void setOrdelLiker(String postId, ArrayList<String> likes)
    {
        StringBuilder sql = new StringBuilder();
        sql.append("UPDATE photo_post SET ");
        sql.append(DBConsts.PHOTO_POST_LIKES + " = ? ");
        sql.append("WHERE post_id = " + postId);
        try(PreparedStatement prSt = dbConnect.prepareStatement(sql.toString()))
        {
            prSt.setString(1,new Gson().toJson(likes));
            prSt.executeUpdate();
        }
        catch (Exception e)
        {
            System.out.println("Problems with liker");
        }
    }
